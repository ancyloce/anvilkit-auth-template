...
insert into users(id,email,password_hash) values
  ($3,'owner@example.com','hash'),
  ($4,'member@example.com','hash'),
  ($5,'target@example.com','hash'),
  ($6,'other-owner@example.com','hash');
insert into tenant_users(tenant_id,user_id,role) values
  ($1,$3,'owner'),
  ($1,$4,'member'),
  ($1,$5,'member'),
  ($2,$6,'owner');`, tenantID, otherTenantID, ownerID, memberID, targetID, otherOwnerID)
	if err != nil {
		t.Fatalf("seed: %v", err)
	}
}
